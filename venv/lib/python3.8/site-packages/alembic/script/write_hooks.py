...
def _run_hooks(path, hook_config):
    ...
    for name in names:
        if not name:
            continue
        opts = {key[len(name) + 1 :]: hook_config[key] for key in hook_config if key.startswith(name + ".")}
        opts["_hook_name"] = name
        try:
            type_ = opts["type"]
        except KeyError as ke:
            compat.raise_(
                util.CommandError("Key %s.type is required for post write hook %r" % (name, name)),
                from_=ke,
            )
        else:
            util.status(
...
def console_scripts(path, options):
    ...
    try:
        ...
        compat.raise_(
            util.CommandError(
                "Key %s.entrypoint is required for post write hook %r" % (options["_hook_name"], options["_hook_name"])
            ),
            from_=ke,
        )
    ...
    subprocess.run(
        [
            sys.executable,
            "-c",
            "import %s; %s()" % (impl.module_name, ".".join((impl.module_name,) + impl.attrs)),
            path,
        ]
        + options.split()
    )
