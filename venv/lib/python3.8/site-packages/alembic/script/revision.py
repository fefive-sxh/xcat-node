...
class CycleDetected(RevisionError):
    ...
    def __init__(self, revisions):
        ...
        super(CycleDetected, self).__init__("%s is detected in revisions (%s)" % (self.kind, ", ".join(revisions)))
...
class RevisionMap(object):
    ...
    def _detect_cycles(self, rev_map, heads, bases, _real_heads, _real_bases):
        ...
        total_space = {
            rev.revision
            for rev in self._iterate_related_revisions(lambda r: r._versioned_down_revisions, heads, map_=rev_map)
        }.intersection(
            rev.revision for rev in self._iterate_related_revisions(lambda r: r.nextrev, bases, map_=rev_map)
        )
        ...
        total_space = {
            rev.revision
            for rev in self._iterate_related_revisions(lambda r: r._all_down_revisions, _real_heads, map_=rev_map)
        }.intersection(
            rev.revision for rev in self._iterate_related_revisions(lambda r: r._all_nextrev, _real_bases, map_=rev_map)
        )
        deleted_revs = set(rev_map.keys()) - total_space
        if deleted_revs:
    ...
    def _normalize_depends_on(self, revisions, map_):
        ...
        for revision in revisions:
            if revision._resolved_dependencies:
                ...
                for rev in self._get_ancestor_nodes([revision], include_dependencies=False, map_=map_):
                    if rev is revision:
                        continue
                    elif rev._resolved_dependencies:
                        normalized_resolved.difference_update(rev._resolved_dependencies)
    ...
    def add_revision(self, revision, _replace=False):
        ...
        for downrev in revision._all_down_revisions:
            if downrev not in map_:
                util.warn("Revision %s referenced from %s is not present" % (downrev, revision))
        ...
        if revision._is_real_head:
            self._real_heads = tuple(
                ...
                if head not in set(revision._all_down_revisions).union([revision.revision])
            ) + (revision.revision,)
        if revision.is_head:
            self.heads = tuple(
                head
                for head in self.heads
                if head not in set(revision._versioned_down_revisions).union([revision.revision])
            ) + (revision.revision,)
    ...
    def _resolve_branch(self, branch_label):
        try:
            ...
            try:
                ...
                util.raise_(
                    ResolutionError("No such branch: '%s'" % branch_label, branch_label),
                    from_=re,
                )
            else:
    ...
    def _revision_for_ident(self, resolved_id, check_branch=None):
        ...
        if revision is False:
            ...
            if not revs:
                ...
                raise ResolutionError(
                    ...
                    "with '%s': %s..." % (resolved_id, ", ".join("'%s'" % r for r in revs[0:3])),
                    resolved_id,
                )
            else:
        ...
        if check_branch and revision is not None:
            if not self._shares_lineage(revision.revision, branch_rev.revision):
                raise ResolutionError(
                    "Revision %s is not a member of branch '%s'" % (revision.revision, check_branch),
                    resolved_id,
                )
        return revision
    ...
    def filter_for_lineage(self, targets, check_against, include_dependencies=False):
        ...
        return [tg for tg in targets if self._shares_lineage(tg, shares, include_dependencies=include_dependencies)]

    def _shares_lineage(self, target, test_against_revs, include_dependencies=False):
        ...
        test_against_revs = [
            ...
            for test_against_rev in util.to_tuple(test_against_revs, default=())
        ]
    ...
    def _resolve_revision_number(self, id_):
        if isinstance(id_, compat.string_types) and "@" in id_:
            ...
            (isinstance(id_, tuple) and id_ and not isinstance(id_[0], compat.string_types))
            or not isinstance(id_, compat.string_types + (tuple,))
        ):
            raise RevisionError(
                "revision identifier %r is not a string; ensure database " "driver settings are correct" % (id_,)
            )
    ...
    def _relative_iterate(
        ...
        revs = list(self._iterate_revisions(from_, to_, inclusive=inclusive, implicit_base=implicit_base))

        if symbol:
            if branch_label:
                symbol_rev = self.get_revision("%s@%s" % (branch_label, symbol))
            else:
        ...
        if is_upwards:
            ...
            if not index and assert_relative_length and len(revs) < abs(relative - reldelta):
                raise RevisionError(
                    "Relative revision %s didn't " "produce %d migrations" % (destination, abs(relative))
                )
        else:
    ...
    def _get_ancestor_nodes(self, targets, map_=None, check=False, include_dependencies=True):
    ...
    def _iterate_revisions(
        ...
        limit_to_lower_branch = isinstance(lower, compat.string_types) and lower.endswith("@base")
        ...
        total_space = set(rev.revision for rev in upper_ancestors).intersection(
            rev.revision
            for rev in self._get_descendant_nodes(
                lowers,
                check=True,
                omit_immediate_dependencies=(select_for_downgrade and requested_lowers),
            )
        )
        ...
        if not total_space:
            ...
            start_from.update(self._get_ancestor_nodes(list(start_from), include_dependencies=True))
        ...
        branch_todo = set(
            ...
            if rev._is_real_branch_point and len(total_space.intersection(rev._all_nextrev)) > 1
        )
        ...
        todo = collections.deque(r for r in uppers if r.revision in total_space)
        ...
        while total_space:
            ...
            if not todo:
                todo.extendleft(
                    sorted(
                        (rev for rev in branch_todo if not rev._all_nextrev.intersection(total_space)),
            ...
            while todo:
                ...
                todo.extendleft(
                    [
                        ...
                        if self._revision_map[downrev] not in branch_todo and downrev in total_space
                    ]
                )
...
class Revision(object):
    ...
    def __init__(self, revision, down_revision, dependencies=None, branch_labels=None):
        ...
        elif dependencies is not None and revision in util.to_tuple(dependencies):
    ...
    def _all_down_revisions(self):
        return util.to_tuple(self.down_revision, default=()) + self._resolved_dependencies
    ...
    def _normalized_down_revisions(self):
        ...
        """
        return util.to_tuple(self.down_revision, default=()) + self._normalized_resolved_dependencies
