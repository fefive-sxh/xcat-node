...
class ScriptDirectory(object):
    ...
    def _load_revisions(self):
        ...
        for vers in paths:
            for file_ in Script._list_py_dir(self, vers):
                ...
                if path in dupes:
                    util.warn("File %s loaded twice! ignoring. Please ensure " "version_locations is unique." % path)
                    continue
                dupes.add(path)
    ...
    def from_config(cls, config):
        ...
        if script_location is None:
            raise util.CommandError("No 'script_location' key " "found in configuration.")
        ...
        if prepend_sys_path:
            sys.path[:0] = list(_split_on_space_comma_colon.split(prepend_sys_path))

        return ScriptDirectory(
            util.coerce_resource_to_filename(script_location),
            file_template=config.get_main_option("file_template", _default_file_template),
    ...
    def get_all_current(self, id_):
        with self._catch_revision_errors():
            ...
            top_revs.update(self.revision_map._get_ancestor_nodes(list(top_revs), include_dependencies=True))
    ...
    def get_base(self):
        ...
        if len(bases) > 1:
            raise util.CommandError("The script directory has multiple bases. " "Please use get_bases().")
        elif bases:
            return bases[0]
        else:
            return None
    ...
    def _downgrade_revs(self, destination, current_rev):
        with self._catch_revision_errors(
            ancestor="Destination %(end)s is not a valid downgrade " "target from current head(s)",
            end=destination,
        ):
            revs = self.revision_map.iterate_revisions(current_rev, destination, select_for_downgrade=True)
    ...
    def _stamp_revs(self, revision, heads):
        with self._catch_revision_errors(
            ...
            for rev in util.to_tuple(revision):
                if rev:
                    filtered_heads.extend(self.revision_map.filter_for_lineage(heads, rev, include_dependencies=True))
            ...
            for dest in dests:
                ...
                if descendants.intersection(filtered_heads):
                    ...
                    step = migration.StampStep((), dest.revision, True, True, self.revision_map)
                    steps.append(step)
                    continue
    ...
    def _generate_template(self, src, dest, **kw):
        util.status(
            "Generating %s" % os.path.abspath(dest), util.template_to_file, src, dest, self.output_encoding, **kw
        )

    def _copy_file(self, src, dest):
        util.status("Generating %s" % os.path.abspath(dest), shutil.copy, src, dest)
    ...
    def _generate_create_date(self):
        if self.timezone is not None:
            ...
            create_date = datetime.datetime.utcnow().replace(tzinfo=tz.tzutc()).astimezone(tzinfo)
        else:
    ...
    def generate_revision(
        ...
        **kw
        ...
        if depends_on:
            with self._catch_revision_errors():
                depends_on = [
                    ...
                    for rev, dep in [(self.revision_map.get_revision(dep), dep) for dep in util.to_list(depends_on)]
                ]
        ...
        self._generate_template(
            ...
            down_revision=revision.tuple_rev_as_scalar(tuple(h.revision if h is not None else None for h in heads)),
        ...
        if branch_labels and not script.branch_labels:
            raise util.CommandError(
                ...
                "'branch_labels' section?" % (script.revision, branch_labels, script.path)
            )
...
class Script(revision.Revision):
    ...
    def __init__(self, module, rev_id, path):
        ...
        super(Script, self).__init__(
            ...
            branch_labels=util.to_tuple(getattr(module, "branch_labels", None), default=()),
            dependencies=util.to_tuple(getattr(module, "depends_on", None), default=()),
        )
