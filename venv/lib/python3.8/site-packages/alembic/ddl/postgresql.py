...
class PostgresqlImpl(DefaultImpl):
    ...
    def compare_server_default(
        ...
        if (
            ...
            and isinstance(metadata_column.server_default.arg, compat.string_types)
            and not re.match(r"^'.*'$", rendered_metadata_default)
        ):
            rendered_metadata_default = "'%s'" % rendered_metadata_default

        return not self.connection.scalar(text("SELECT %s = %s" % (conn_col_default, rendered_metadata_default)))
    ...
    def autogen_column_reflect(self, inspector, table, column_info):
        if column_info.get("default") and isinstance(column_info["type"], (INTEGER, BIGINT)):
            seq_match = re.match(r"nextval\('(.+?)'::regclass\)", column_info["default"])
            if seq_match:
    ...
    def correct_for_autogen_constraints(
        ...
        doubled_constraints = set(index for index in conn_indexes if index.info.get("duplicates_constraint"))
    ...
    def _render_HSTORE_type(self, type_, autogen_context):
        return render._render_type_w_subtype(type_, autogen_context, "text_type", r"(.+?\(.*text_type=)")

    def _render_ARRAY_type(self, type_, autogen_context):
        return render._render_type_w_subtype(type_, autogen_context, "item_type", r"(.+?\()")

    def _render_JSON_type(self, type_, autogen_context):
        return render._render_type_w_subtype(type_, autogen_context, "astext_type", r"(.+?\(.*astext_type=)")
...
def visit_identity_column(element, compiler, **kw):
    ...
    if element.default is None:
        ...
        # alter identity
        diff, _, _ = element.impl._compare_identity_default(element.default, element.existing_server_default)
        identity = element.default
        for attr in sorted(diff):
            if attr == "always":
                text += "SET GENERATED %s " % ("ALWAYS" if identity.always else "BY DEFAULT")
            else:
...
class CreateExcludeConstraintOp(ops.AddConstraintOp):
    ...
    def __init__(self, constraint_name, table_name, elements, where=None, schema=None, _orig_constraint=None, **kw):
...
def _exclude_constraint(constraint, autogen_context, alter):
    ...
    if alter:
        ...
        args = [
            "(%s, %r)" % (_render_potential_column(sqltext, autogen_context), opstring)
            for sqltext, name, opstring in constraint._render_exprs
        ]
        if constraint.where is not None:
            args.append("where=%s" % render._render_potential_expr(constraint.where, autogen_context))
