...
class MySQLImpl(DefaultImpl):
    ...
    def compare_server_default(
        ...
        if (
            ...
            rendered_inspector_default = (
                re.sub(r"^'|'$", "", rendered_inspector_default) if rendered_inspector_default is not None else None
            )
            ...
            return re.sub(r"(.*?)(?:\(\))?$", r"\1", rendered_inspector_default.lower()) != re.sub(
    ...
    def correct_for_autogen_foreignkeys(self, conn_fks, metadata_fks):
        conn_fk_by_sig = dict((compare._fk_constraint_sig(fk).sig, fk) for fk in conn_fks)
        metadata_fk_by_sig = dict((compare._fk_constraint_sig(fk).sig, fk) for fk in metadata_fks)
...
class MySQLChangeColumn(AlterColumn):
    def __init__(
        ...
        if type_ is None:
            raise util.CommandError("All MySQL CHANGE/MODIFY COLUMN operations " "require the existing type.")
...
def _mysql_colspec(compiler, nullable, server_default, type_, autoincrement, comment):
    spec = "%s %s" % (
