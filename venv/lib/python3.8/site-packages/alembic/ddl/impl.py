...
class DefaultImpl(with_metaclass(ImplMeta)):
    ...
    def __init__(
        ...
        if self.literal_binds:
            if not self.as_sql:
                raise util.CommandError("Can't use literal_binds setting without as_sql mode")
    ...
    def _exec(
        ...
        if self.as_sql:
            ...
            self.static_output(
                text_type(construct.compile(dialect=self.dialect, **compile_kw)).replace("\t", "    ").strip()
    ...
    def alter_column(
        ...
        if server_default is not False:
            kw = {}
            if sqla_compat._server_default_is_computed(server_default, existing_server_default):
                cls_ = base.ComputedColumnDefault
            elif sqla_compat._server_default_is_identity(server_default, existing_server_default):
    ...
    def create_table(self, table):
        table.dispatch.before_create(table, self.connection, checkfirst=False, _ddl_runner=self)
        self._exec(schema.CreateTable(table))
        table.dispatch.after_create(table, self.connection, checkfirst=False, _ddl_runner=self)
        for index in table.indexes:
            self._exec(schema.CreateIndex(index))

        with_comment = self.dialect.supports_comments and not self.dialect.inline_comments
    ...
    def bulk_insert(self, table, rows, multiinsert=True):
        ...
        if self.as_sql:
                ...
                self._exec(
                    sqla_compat._insert_inline(table).values(
                        **dict(
                            (
                                k,
                                sqla_compat._literal_bindparam(k, v, type_=table.c[k].type)
            ...
            if rows:
                if multiinsert:
                    self._exec(sqla_compat._insert_inline(table), multiparams=rows)
                else:
                    for row in rows:
                        self._exec(sqla_compat._insert_inline(table).values(**row))
    ...
    def render_ddl_sql_expr(self, expr, is_server_default=False, **kw):
        ...
        compile_kw = dict(compile_kwargs={"literal_binds": True, "include_table": False})
...
def _compare_identity_options(attributes, metadata_io, inspector_io, default_io):
