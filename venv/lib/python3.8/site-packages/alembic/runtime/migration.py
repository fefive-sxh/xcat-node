...
class MigrationContext(object):
    ...
    def __init__(self, dialect, connection, opts, environment_context=None):
        ...
        log.info(
            "Will assume %s DDL.",
            "transactional" if self.impl.transactional_ddl else "non-transactional",
        )
...
class HeadMaintainer(object):
    ...
    def _delete_version(self, version):
        ...
        ret = self.context.impl._exec(
            self.context._version.delete().where(
                self.context._version.c.version_num == literal_column("'%s'" % version)
            )
        )
        if not self.context.as_sql and self.context.dialect.supports_sane_rowcount and ret.rowcount != 1:
            raise util.CommandError(
                "Online migration expected to match one "
                "row when deleting '%s' in '%s'; "
                "%d found" % (version, self.context.version_table, ret.rowcount)
            )
    ...
    def _update_version(self, from_, to_):
        ...
        ret = self.context.impl._exec(
            ...
            .where(self.context._version.c.version_num == literal_column("'%s'" % from_))
        )
        if not self.context.as_sql and self.context.dialect.supports_sane_rowcount and ret.rowcount != 1:
            raise util.CommandError(
                "Online migration expected to match one "
                "row when updating '%s' to '%s' in '%s'; "
                "%d found" % (from_, to_, self.context.version_table, ret.rowcount)
            )
...
class MigrationInfo(object):
    ...
    def __init__(self, revision_map, is_upgrade, is_stamp, up_revisions, down_revisions):
...
class RevisionStep(MigrationStep):
    ...
    def __eq__(self, other):
        return (
            isinstance(other, RevisionStep) and other.revision == self.revision and self.is_upgrade == other.is_upgrade
        )
