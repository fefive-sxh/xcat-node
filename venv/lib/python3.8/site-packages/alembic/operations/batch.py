    ...
    def add_column(self, *arg, **kw):
        if ("insert_before" in kw or "insert_after" in kw) and not self._should_recreate():
            raise exc.CommandError(
                "Can't specify insert_before or insert_after when using " "ALTER; please specify recreate='always'"
            )
...
class ApplyBatchImpl(object):
    ...
    def _adjust_self_columns_for_partial_reordering(self):
        ...
        sorted_ = list(topological.sort(pairs, col_by_idx, deterministic_order=True))
        ...
        try:
            op_impl._exec(
                _insert_inline(self.new_table).from_select(
                    ...
                    _select(*[transfer["expr"] for transfer in self.column_transfers.values() if "expr" in transfer]),
                )
            )
            ...
            op_impl.rename_table(self.temp_table_name, self.table.name, schema=self.table.schema)
    ...
    def _setup_dependencies_for_add_column(self, colname, insert_before, insert_after):
            ...
            if insert_before:
                if not insert_after:
                    if insert_before in col_indexes:
                        ...
                        insert_after = dict((b, a) for a, b in self.add_col_ordering)[insert_before]
    ...
    def add_column(self, table_name, column, insert_before=None, insert_after=None, **kw):
        self._setup_dependencies_for_add_column(column.name, insert_before, insert_after)
