...
class SchemaObjects(object):
    ...
    def foreign_key_constraint(
        ...
        **dialect_kw
        ...
        t1 = sa_schema.Table(source, m, *[sa_schema.Column(n, NULLTYPE) for n in t1_cols], schema=source_schema)
    ...
    def unique_constraint(self, name, source, local_cols, schema=None, **kw):
        t = sa_schema.Table(
            source, self.metadata(), *[sa_schema.Column(n, NULLTYPE) for n in local_cols], schema=schema
        )
        kw["name"] = name
    ...
    def generic_constraint(self, name, table_name, type_, schema=None, **kw):
        ...
        try:
            ...
            raise_(
                TypeError("'type' can be one of %s" % ", ".join(sorted(repr(x) for x in types))),
                from_=ke,
            )
        else:
    ...
    def index(self, name, tablename, columns, schema=None, **kw):
        t = sa_schema.Table(tablename or "no_table", self.metadata(), schema=schema)
        idx = sa_schema.Index(name, *[util.sqla_compat._textual_index_column(t, n) for n in columns], **kw)
        return idx
