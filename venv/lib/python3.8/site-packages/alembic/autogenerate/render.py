...
def _render_python_into_templatevars(autogen_context, migration_script, template_args):
...
def _render_create_table_comment(autogen_context, op):
    ...
    return templ.format(
        ...
        existing="%r" % op.existing_comment if op.existing_comment is not None else None,
...
def _add_table(autogen_context, op):
    ...
    args = [col for col in [_render_column(col, autogen_context) for col in table.columns] if col] + sorted(
        [
            rcons
            for rcons in [_render_constraint(cons, autogen_context) for cons in table.constraints]
...
def _add_index(autogen_context, op):
    ...
    if has_batch:
        tmpl = "%(prefix)screate_index(%(name)r, [%(columns)s], " "unique=%(unique)r%(kwargs)s)"
    else:
        tmpl = "%(prefix)screate_index(%(name)r, %(table)r, [%(columns)s], " "unique=%(unique)r%(schema)s%(kwargs)s)"
    ...
    text = tmpl % {
        ...
        "schema": (", schema=%r" % _ident(index.table.schema)) if index.table.schema else "",
        "kwargs": (
            ", "
            + ", ".join(
                ["%s=%s" % (key, _render_potential_expr(val, autogen_context)) for key, val in index.kwargs.items()]
            )
        )
        if len(index.kwargs)
        else "",
...
def _drop_index(autogen_context, op):
    ...
    if has_batch:
        ...
        tmpl = "%(prefix)sdrop_index(%(name)r, " "table_name=%(table_name)r%(schema)s)"
...
def _drop_constraint(autogen_context, op):
    ...
    if autogen_context._has_batch:
        ...
        template = "%(prefix)sdrop_constraint" "(%(name)r, '%(table_name)s'%(schema)s, type_=%(type)r)"
...
def _alter_column(autogen_context, op):
    ...
    if server_default is False and existing_server_default:
        rendered = _render_server_default(existing_server_default, autogen_context)
...
def _render_potential_expr(value, autogen_context, wrap_in_text=True, is_server_default=False):
...
def _should_render_server_default_positionally(server_default):
    return sqla_compat._server_default_is_computed(server_default) or sqla_compat._server_default_is_identity(
        server_default
    )
...
def _render_ARRAY_type(type_, autogen_context):
    return _render_type_w_subtype(type_, autogen_context, "item_type", r"(.+?\()")
...
def _render_type_w_subtype(type_, autogen_context, attrname, regexp, prefix=None):
...
def _render_primary_key(constraint, autogen_context):
    ...
    return "%(prefix)sPrimaryKeyConstraint(%(args)s)" % {
        "prefix": _sqlalchemy_autogenerate_prefix(autogen_context),
        "args": ", ".join(
            [repr(c.name) for c in constraint.columns] + ["%s=%s" % (kwname, val) for kwname, val in opts]
        ),
    }
...
def _render_foreign_key(constraint, autogen_context):
    ...
    return "%(prefix)sForeignKeyConstraint([%(cols)s], " "[%(refcols)s], %(args)s)" % {
        "prefix": _sqlalchemy_autogenerate_prefix(autogen_context),
        "cols": ", ".join("%r" % _ident(f.parent.name) for f in constraint.elements),
        "refcols": ", ".join(repr(_fk_colspec(f, apply_metadata_schema)) for f in constraint.elements),
...
def _render_check_constraint(constraint, autogen_context):
    ...
    return "%(prefix)sCheckConstraint(%(sqltext)s%(opts)s)" % {
        ...
        "sqltext": _render_potential_expr(constraint.sqltext, autogen_context, wrap_in_text=False),
    }
