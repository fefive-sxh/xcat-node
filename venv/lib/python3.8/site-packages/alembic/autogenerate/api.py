    ...
    def __init__(self, migration_context, metadata=None, opts=None, autogenerate=True):
        ...
        if autogenerate and migration_context is not None and migration_context.as_sql:
            raise util.CommandError(
                "autogenerate can't use as_sql=True as it prevents querying " "the database for schema information"
            )
        ...
        if autogenerate and metadata is None and migration_context is not None and migration_context.script is not None:
            raise util.CommandError(
                ...
                "a MetaData object or sequence of objects to the context." % (migration_context.script.env_py_location)
            )
        ...
        for m in util.to_list(self.metadata):
            ...
            if intersect:
                raise ValueError(
                    ...
                    "MetaData objects: %s" % (", ".join('"%s"' % key for key in sorted(intersect)))
                )
...
class RevisionContext(object):
    ...
    def _run_environment(self, rev, migration_context, autogenerate):
        ...
        self._last_autogen_context = autogen_context = AutogenContext(migration_context, autogenerate=autogenerate)
