...
def _autogen_for_tables(autogen_context, upgrade_ops, schemas):
    ...
    for schema_name in schemas:
        ...
        conn_table_names.update(
            (schema_name, tname)
            for tname in tables
            if autogen_context.run_name_filters(tname, "table", {"schema_name": schema_name})
        )
...
def _compare_tables(
    ...
    metadata_table_names_no_dflt_schema = OrderedSet(
        [(schema if schema != default_schema else None, tname) for schema, tname in metadata_table_names]
    )
    ...
    tname_to_table = dict(
        (
            no_dflt_schema,
            autogen_context.table_key_to_table[sa_schema._get_table_key(tname, schema)],
        )
        for no_dflt_schema, (schema, tname) in zip(metadata_table_names_no_dflt_schema, metadata_table_names)
    )
...
def _make_unique_constraint(params, conn_table):
    uq = sa_schema.UniqueConstraint(*[conn_table.c[cname] for cname in params["column_names"]], name=params["name"])
...
def _compare_columns(
    ...
    metadata_cols_by_name = dict((c.name, c) for c in metadata_table.c if not c.system)
    conn_col_names = dict(
        (c.name, c)
        for c in conn_table.c
        if autogen_context.run_name_filters(c.name, "column", {"table_name": tname, "schema_name": schema})
    )
    metadata_col_names = OrderedSet(sorted(metadata_cols_by_name))

    for cname in metadata_col_names.difference(conn_col_names):
        if autogen_context.run_object_filters(metadata_cols_by_name[cname], cname, "column", False, None):
            modify_table_ops.ops.append(
                ops.AddColumnOp.from_column_and_tablename(schema, tname, metadata_cols_by_name[cname])
            )
    ...
    for colname in metadata_col_names.intersection(conn_col_names):
        ...
        if not autogen_context.run_object_filters(metadata_col, colname, "column", False, conn_col):
            continue
    ...
    for cname in set(conn_col_names).difference(metadata_col_names):
        if autogen_context.run_object_filters(conn_table.c[cname], cname, "column", True, None):
            modify_table_ops.ops.append(ops.DropColumnOp.from_column_and_tablename(schema, tname, conn_table.c[cname]))
...
def _compare_indexes_and_uniques(autogen_context, modify_ops, schema, tname, conn_table, metadata_table):
    ...
    if metadata_table is not None:
        metadata_unique_constraints = set(
            uq for uq in metadata_table.constraints if isinstance(uq, sa_schema.UniqueConstraint)
        )
    ...
    if conn_table is not None:
        ...
        if is_drop_table:
            ...
            conn_uniques = set(_make_unique_constraint(uq_def, conn_table) for uq_def in conn_uniques)
    ...
    # interchangeably
    metadata_unique_constraints = set(_uq_constraint_sig(uq) for uq in metadata_unique_constraints)
    ...
    metadata_names = dict(
        ...
        if isinstance(c, _ix_constraint_sig) or sqla_compat._constraint_is_named(c.const, autogen_context.dialect)
    )
    ...
    conn_names = dict((c.name, c) for c in conn_unique_constraints.union(conn_indexes) if c.name is not None)
    ...
    metadata_uniques_by_sig = dict((uq.sig, uq) for uq in metadata_unique_constraints)
    metadata_indexes_by_sig = dict((ix.sig, ix) for ix in metadata_indexes)
    unnamed_metadata_uniques = dict(
        (uq.sig, uq)
        for uq in metadata_unique_constraints
        if not sqla_compat._constraint_is_named(uq.const, autogen_context.dialect)
    )
    ...
    for removed_name in sorted(set(conn_names).difference(metadata_names)):
        ...
        if not conn_obj.is_index and conn_obj.sig in unnamed_metadata_uniques:
            ...
            if conn_obj.sig not in metadata_indexes_by_sig and conn_obj.sig not in metadata_uniques_by_sig:
    ...
    for existing_name in sorted(set(metadata_names).intersection(conn_names)):
        ...
        if conn_obj.is_index != metadata_obj.is_index:
            ...
            if conn_obj.is_unique != metadata_obj.is_unique:
                msg.append(" unique=%r to unique=%r" % (conn_obj.is_unique, metadata_obj.is_unique))
            if conn_obj.sig != metadata_obj.sig:
                msg.append(" columns %r to %r" % (conn_obj.sig, metadata_obj.sig))
...
def _correct_for_uq_duplicates_uix(
    ...
    metadata_cons_names = [
        (sqla_compat._get_constraint_final_name(cons, dialect), cons) for cons in metadata_unique_constraints
    ]

    metadata_uq_names = set(name for name, cons in metadata_cons_names if name is not None)

    unnamed_metadata_uqs = set([_uq_constraint_sig(cons).sig for name, cons in metadata_cons_names if name is None])

    metadata_ix_names = set(
        [sqla_compat._get_constraint_final_name(cons, dialect) for cons in metadata_indexes if cons.unique]
    )

    # for reflection side, names are in their final database form
    # already since they're from the database
    conn_ix_names = dict((cons.name, cons) for cons in conn_indexes if cons.unique)

    uqs_dupe_indexes = dict((cons.name, cons) for cons in conn_unique_constraints if cons.info["duplicates_index"])

    for overlap in uqs_dupe_indexes:
        if overlap not in metadata_uq_names:
            if _uq_constraint_sig(uqs_dupe_indexes[overlap]).sig not in unnamed_metadata_uqs:
...
def _compare_nullable(
    ...
    if conn_col_nullable is not metadata_col_nullable:
        if (
            sqla_compat._server_default_is_computed(metadata_col.server_default, conn_col.server_default)
            and sqla_compat._nullability_might_be_unset(metadata_col)
            or (sqla_compat._server_default_is_identity(metadata_col.server_default, conn_col.server_default))
        ):
            log.info(
...
def _compare_type(
    ...
    isdiff = autogen_context.migration_context._compare_type(conn_col, metadata_col)
...
def _render_server_default_for_compare(metadata_default, metadata_col, autogen_context):
    rendered = _user_defined_render("server_default", metadata_default, autogen_context)
...
def _compare_identity_default(
    ...
    diff, ignored_attr, is_alter = impl._compare_identity_default(metadata_col.server_default, conn_col.server_default)
...
def _compare_server_default(
    ...
    if sqla_compat._server_default_is_identity(metadata_default, conn_col_default):
        ...
        if is_alter:
            ...
            if diff:
                log.info(
                    "Detected server default on column '%s.%s': " "identity options attributes %s",
                    tname,
                    cname,
                    sorted(diff),
                )
    else:
        rendered_metadata_default = _render_server_default_for_compare(metadata_default, metadata_col, autogen_context)

        rendered_conn_default = conn_col_default.arg.text if conn_col_default else None
...
def _compare_foreign_keys(
    ...
    inspector = autogen_context.inspector
    metadata_fks = set(fk for fk in metadata_table.constraints if isinstance(fk, sa_schema.ForeignKeyConstraint))
    ...
    metadata_fks = set(_fk_constraint_sig(fk, include_options=backend_reflects_fk_options) for fk in metadata_fks)

    conn_fks = set(_fk_constraint_sig(fk, include_options=backend_reflects_fk_options) for fk in conn_fks)

    conn_fks_by_sig = dict((c.sig, c) for c in conn_fks)
    metadata_fks_by_sig = dict((c.sig, c) for c in metadata_fks)

    metadata_fks_by_name = dict((c.name, c) for c in metadata_fks if c.name is not None)
    conn_fks_by_name = dict((c.name, c) for c in conn_fks if c.name is not None)
