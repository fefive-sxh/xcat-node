...
class ModuleClsProxy(with_metaclass(_ModuleClsMeta)):
    ...
    def _add_proxied_attribute(cls, methname, globals_, locals_, attr_names):
        if not methname.startswith("_"):
            ...
            if callable(meth):
                locals_[methname] = cls._create_method_proxy(methname, globals_, locals_)
            else:
        ...
        def _name_error(name, from_):
            raise_(
                NameError(
                    ...
                    "Try placing this code inside a callable." % (name, cls.__name__)
                ),
                from_=from_,
            )
...
class immutabledict(dict):
    ...
    __delitem__ = __setitem__ = __setattr__ = clear = pop = popitem = setdefault = update = _immutable
...
class Dispatcher(object):
    ...
    def branch(self):
        ...
        d = Dispatcher()
        if self.uselist:
            d._registry.update((k, [fn for fn in self._registry[k]]) for k in self._registry)
        else:
