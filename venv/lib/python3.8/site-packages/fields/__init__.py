...
def make_init_func(
    ...
    for var in fields:
        if var in defaults:
            ...
            raise ValueError(
                "Cannot have positional fields after fields with defaults. "
...
def tuple_sealer(fields, defaults):
    ...
    def __repr__(self):
        return "{0}({1})".format(self.__class__.__name__, ", ".join(a + "=" + repr(getattr(self, a)) for a in fields))
...
class _Factory(type):
    ...
    def __getattr__(cls, name):
        ...
        if name in cls.__defaults:
            raise TypeError("Field %r is already specified with a default value (%r)." % (name, cls.__defaults[name]))
    ...
    def __new__(
        ...
        if not bases:
            ...
            return type.__new__(
                ...
                if all_fields
                else "Fields<{0}>".format(sealer.__name__),
                bases,
                dict(
            ...
            for pos, names in enumerate(zip(*[k.__all_fields for k in bases if isinstance(k, _Factory)])):
                if names:
                    if len(set(names)) != 1:
                        raise TypeError(
                            "Field layout conflict: fields in position {0} have different names: {1}".format(
                                pos, ", ".join(repr(name) for name in names)
                            )
                        )

            return type(
                name, tuple(~k if isinstance(k, _Factory) else k for k in bases), {} if namespace is None else namespace
            )
